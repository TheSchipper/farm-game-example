...
const MINUTES_PER_HOURS: int = 60
...
func set_initial_time() -> void:
	var initial_total_minutes = initial_day * MINUTES_PER_DAY + (initial_hour * MINUTES_PER_HOURS) + initial_minute
...
func recalculate_time() -> void:
	...
	var hour: int = int(current_day_minutes / MINUTES_PER_HOURS)
	var minute: int = int(current_day_minutes % MINUTES_PER_HOURS)
